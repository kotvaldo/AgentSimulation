...
{
	private Queue nonStartedFurniture;
	public ManagerNabytku(int id, Simulation mySim, Agent myAgent)
	{
		super(id, mySim, myAgent);
		init();
		nonStartedFurniture = new Queue(null);
	}
	...
	public void processNoticeSpracujObjednavku(MessageForm message)
	{
		...
		if(this.nonStartedFurniture.getQueue().isEmpty()) {
			for (Furniture furniture : msg.getOrder().getFurnitureList()) {
				MyMessage workerForCuttingMessage = new MyMessage(msg);
				workerForCuttingMessage.setCode(Mc.rVyberPracovnikaRezanie);
				workerForCuttingMessage.setAddressee(_mySim.findAgent(Id.agentPracovnikov));
				request(workerForCuttingMessage);

				MyMessage workplaceMessage = new MyMessage(msg);
				workplaceMessage.setCode(Mc.rDajVolnePracovneMiesto);
				workplaceMessage.setAddressee(_mySim.findAgent(Id.agentPracovisk));
				request(workplaceMessage);
